...
nukar1_rtr = conn.network.find_router('nukar1-rtr')
if nukar1_rtr is None:
        print("router already does not exist")
else:
        conn.network.delete_router(nukar1_rtr)
        print("router has been deleted")
nukar1_subnet = conn.network.find_subnet('nukar1-subnet')
if  nukar1_subnet is None:
        print("subnet already does not exist")
else:
        conn.network.delete_subnet(nukar1_subnet)
        print("subnet has been deleted")
nukar1_network = conn.network.find_network('nukar1-network')
if nukar1_network is None:
        print("network already does not exist")
else:
        conn.network.delete_network(nukar1_network)
...
def status():
	...
	# checks the status of the created servers
...
#note..
# most of the script works, however trying to run the functions
# of the script individually does not work.
# whatever the function it runs through the entire script each time.
